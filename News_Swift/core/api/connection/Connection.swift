...
class Connect {
    var url = "https://staging-news-api.lavia.vn/"
...
//    }
    func fetchGet(parram : [String : Any]?,endUrl : String) {
        AF.request("\(url)\(endUrl)",method: .get,parameters: parram , encoding: URLEncoding.default,headers: nil,interceptor: nil).responseJSON { (response) in
            if let jsonObj = response.value as? [String : Any] {
                self.callBack?(jsonObj as? [String : Any] )
            }
        }
